...
type Sections<'data> = &'data [Section64<crate::macho::Endianness>];
type SymbolCommand<'data> = SymtabCommand<crate::macho::Endianness>;
...
pub(crate) struct MachOFile<'data> {
    ...
    #[debug(skip)]
    pub(crate) sections: Sections<'data>,
    #[debug(skip)]
    pub(crate) symbols: SymbolCommand<'data>,
    pub(crate) flags: u32,
}
    ...
    pub(crate) fn parse(data: &'data [u8]) -> Result<Self> {
        ...
        while let Some(command) = commands.next()? {
            if let Some(symtab_command) = command.symtab()? {
                ...
                symbols = Some(*symtab_command);

                let ss = symtab_command.symbols::<macho::MachHeader64<_>, _>(LE, data)?;
                for symbol in ss.iter() {
                    let name = symbol.name(LE, ss.strings())?;
                    println!("symbol: {}", String::from_utf8_lossy(name));
                }
